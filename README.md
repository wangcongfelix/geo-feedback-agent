# GeoFeedback Agent v3.0

面向地图、导航与 GIS 产品经理的微信反馈归档和 AI 诊断工作台。

它把微信群截图中的零散建议转化为可追溯的反馈档案：浏览器本地识别文字、截取消息旁头像作为身份标识、自动分类分析，再由产品经理复核并生成标准问题单。

## v3.0 展示重点

- **反馈档案看板**：集中查看建议文本、头像标识、来源截图、反馈时间、类型和处理状态。
- **截图导入与粘贴**：支持选择、拖拽和 `Ctrl+V` 粘贴微信截图，并在浏览器本地完成 OCR。
- **头像身份留档**：从截图侧边截取头像，可切换左右消息方向、上下微调，并添加可选人工备注。
- **自动分类与人工复核**：复用原有 AI 诊断、Human-in-the-loop 审核和标准问题单生成流程。
- **时间与类型统计**：用近 7 日、近 30 日趋势观察功能建议、数据问题、体验问题和 Bug 的变化。

v3.0 是产品界面版本，不是 Prompt V3。AI 诊断仍使用经过 Golden Set 回归验证的稳定 Prompt V1。
...
![反馈档案与趋势看板](public/screenshots/01-v3-dashboard.png)

### 微信截图导入与头像留档

![微信截图导入与头像留档](public/screenshots/02-v3-screenshot-import.png)

### AI 分类与产品经理人工复核

![AI 分类与产品经理人工复核](public/screenshots/03-v3-human-review.png)

## 产品迭代过程

| 产品版本 | 核心问题 | 主要能力 | 迭代判断 |
| --- | --- | --- | --- |
| v1.0 | 零散反馈难以直接进入研发流程 | 单条 AI 结构化诊断、人工审核、标准问题单 | 先验证 AI 能否辅助产品经理完成反馈分拣 |
| v2.0 | 单条建议脱离来源后难以回溯 | 反馈档案、截图凭证、时间与类型看板 | 从“处理一条反馈”扩展为“沉淀反馈证据” |
| v3.0 | 微信截图录入仍需要手工补身份信息 | 拖拽/粘贴截图、本地 OCR、头像裁剪留档、可选人工备注 | 保留原工作流，只补齐截图到档案的关键链路 |

这三轮都遵循“小步验证”的原则：不重做稳定的 AI 诊断与人工复核流程，只围绕最影响产品经理效率的缺口增加能力。
...
真实上传的截图、OCR 和头像裁剪都只在浏览器本地处理，不上传服务器，也不传给 DeepSeek；只有识别后的文字会进入诊断接口。v3.0 档案页的初始记录使用示例数据，新识别档案会写入浏览器本地存储，不使用数据库。
